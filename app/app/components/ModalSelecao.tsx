...
export default function ModalSelecao({ uid, galeriaId, titulo, selecao, onFechar }: Props) {
  ...
  useEffect(() => {
    ...
    function teclado(e: KeyboardEvent) {
      ...
      if (aberta === null || !escolhidas.length) return;
      if (e.key === "ArrowRight") setAberta((v) => v === null ? null : (v + 1) % escolhidas.length);
      if (e.key === "ArrowLeft") setAberta((v) => v === null ? null : (v - 1 + escolhidas.length) % escolhidas.length);
    }
        ...
        <div className="ms-body">
          <div className="ms-actions">
            ...
            <div className="ms-actions-right">
              {baixando && <button className="ms-btn secondary" onClick={cancelarDownload}>Cancelar</button>}
              <button className="ms-btn" onClick={() => void baixarTodas()} disabled={!escolhidas.length || baixando}>{baixando ? `Baixando ${progresso}/${escolhidas.length}` : "Baixar selecionadas"}</button>
            </div>
          </div>
          {falhasDownload > 0 && <div className="ms-warning" style={{marginBottom:12}}>{falhasDownload} arquivo{falhasDownload === 1 ? " não pôde" : "s não puderam"} ser baixado{falhasDownload === 1 ? "" : "s"}.</div>}
          {carregando ? <div className="ms-empty">Preparando miniaturas da seleção…</div> : erro ? <div className="ms-error">{erro}</div> : escolhidas.length ? <div className="ms-grid">{escolhidas.map((nome, i) => {
            const foto = fotoPorNome(nome);
            const temComentario = Boolean(selecao.comentarios?.[nome]?.trim());
            return <div className="ms-photo" key={nome} role="button" tabIndex={foto ? 0 : -1} aria-label={`Abrir ${nome}`} onClick={()=>foto&&setAberta(i)} onKeyDown={(e)=>{if(foto&&(e.key==="Enter"||e.key===" ")){e.preventDefault();setAberta(i)}}}>{foto ? <img src={foto.thumb || foto.original} alt={nome} loading="lazy" onError={(e)=>{if(e.currentTarget.src!==foto.original)e.currentTarget.src=foto.original}}/> : <div className="ms-missing">Foto indisponível</div>}{temComentario&&<span className="ms-comment-dot" title="Esta foto tem comentário">1</span>}</div>;
          ...
          {comentarios.length > 0 && <div className="ms-section"><h3>Comentários</h3>{comentarios.map(([nome,texto]) => {
            ...
            return <div className={"ms-comment"+(podeAbrir?" has-photo":"")} key={nome} onClick={()=>podeAbrir&&abrirNome(nome)}><div className="ms-file">{nome}{podeAbrir ? " · abrir foto" : ""}</div><div className="ms-text">{texto}</div></div>;
          })}</div>}
        </div>
      </section>

      {aberta !== null && fotoAberta && <div className="ms-lightbox" role="dialog" aria-modal="true" aria-label={`Foto ${fotoAberta.nome}`} onMouseDown={(e)=>{if(e.target===e.currentTarget)setAberta(null)}}>
        <button className="ms-lb-close" onClick={()=>setAberta(null)} aria-label="Fechar foto">×</button>
        {escolhidas.length > 1 && <button className="ms-lb-nav prev" onClick={()=>setAberta((aberta-1+escolhidas.length)%escolhidas.length)} aria-label="Foto anterior">‹</button>}
        <img src={fotoAberta.original} alt={fotoAberta.nome}/>
        {escolhidas.length > 1 && <button className="ms-lb-nav next" onClick={()=>setAberta((aberta+1)%escolhidas.length)} aria-label="Próxima foto">›</button>}
