...
type T = "painel" | "galerias" | "selecoes" | "clientes" | "perfil" | "config" | "sair" | "menu" | "fechar";
...
function Icone({ tipo }: { tipo: T }) {
  ...
  if (tipo === "perfil") return <svg {...common} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8"><circle cx="12" cy="8" r="3.2"/><path d="M5.5 20c.6-4.2 2.8-6.3 6.5-6.3s5.9 2.1 6.5 6.3"/></svg>;
...
export default function MenuFotografo() {
  ...
  const itens: { rota: string; label: string; tipo: T; exato?: boolean }[] = [
    ...
    { rota: "/perfil", label: "Perfil", tipo: "perfil", exato: true },
  ...
  return <>
    ...
    <aside ref={sidebarRef} id="mf-sidebar" tabIndex={-1} className={"mf-side" + (drawer ? " aberta" : "")} aria-label="Navegação do fotógrafo" aria-hidden={menuOculto || undefined} inert={menuOculto || undefined}>
      ...
      <div className="mf-bottom"><div className="mf-user" title={email || nome}><div className="mf-avatar" aria-hidden="true">{logo?<img src={logo} alt=""/>:inicial}</div><div className="mf-user-info"><div className="mf-user-name">{nome || "Fotógrafo"}</div><div className="mf-user-email">{email}</div></div></div><button className="mf-sair" onClick={sair} aria-label="Sair"><span className="mf-ic"><Icone tipo="sair"/></span><span className="mf-tx">Sair</span></button></div>
    </aside>
  </>;
}
