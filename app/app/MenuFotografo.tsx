...
type T = "painel" | "galerias" | "selecoes" | "clientes" | "config" | "sair" | "menu" | "fechar";
...
export default function MenuFotografo() {
  ...
  useEffect(() => {
    ...
    void (async () => {
      ...
      if (ativo) setNome((perfil?.nome_estudio as string | null)?.trim() || e.split("@")[0] || "Fotógrafo");
    })();
    return () => { ativo = false; };
  }, [supabase]);
  ...
  useEffect(() => {
    ...
    function teclado(event: KeyboardEvent) {
      ...
      const ultimoFoco = focaveis[focaveis.length - 1];
      if (event.shiftKey && document.activeElement === primeiroFoco) {
        event.preventDefault();
        ultimoFoco.focus();
      } else if (!event.shiftKey && document.activeElement === ultimoFoco) {
        event.preventDefault();
        primeiroFoco.focus();
      }
    }
  ...
  return <>
    ...
    <aside ref={sidebarRef} id="mf-sidebar" tabIndex={-1} className={"mf-side" + (drawer ? " aberta" : "")} aria-label="Navegação do fotógrafo" aria-hidden={menuOculto || undefined} inert={menuOculto || undefined}>
      ...
      <nav className="mf-nav">{itens.map((item) => { const ativo = item.exato ? pathname === item.rota : pathname === item.rota || pathname.startsWith(item.rota + "/"); return <button key={item.rota} className={"mf-item" + (ativo ? " ativo" : "")} onClick={() => router.push(item.rota)} aria-current={ativo ? "page" : undefined} aria-label={item.label} title={item.label}><span className="mf-ic"><Icone tipo={item.tipo}/></span><span className="mf-tx">{item.label}</span></button>; })}</nav>
      <div className="mf-bottom"><div className="mf-user" title={email || nome}><div className="mf-avatar" aria-hidden="true">{inicial}</div><div className="mf-user-info"><div className="mf-user-name">{nome || "Fotógrafo"}</div><div className="mf-user-email">{email}</div></div></div><button className="mf-sair" onClick={sair} aria-label="Sair"><span className="mf-ic"><Icone tipo="sair"/></span><span className="mf-tx">Sair</span></button></div>
    </aside>
  </>;
}
