...
export default function AdminActions({ userId, temStripe, suspenso = false, takedownPublico = false }: { userId: string; temStripe: boolean; suspenso?: boolean; takedownPublico?: boolean }) {
  ...
  return <section className="aa-card">
    <style>{`.aa-card{margin-top:14px;background:linear-gradient(180deg,#15152c,#F3EFF9);border:1px solid #2b2e49;border-radius:16px;padding:18px}.aa-title{font-size:14px;font-weight:700;margin-bottom:6px}.aa-sub{font-size:11px;color:#777e9d;margin-bottom:14px}.aa-grid{display:grid;grid-template-columns:1fr 1fr;gap:12px}.aa-field{display:grid;gap:6px}.aa-field label{font-size:10px;color:#777e9d}.aa-field select,.aa-field textarea{width:100%;box-sizing:border-box;background:#0d0e1d;border:1px solid #2b2e49;border-radius:10px;color:#21253A;padding:10px;font:12px inherit}.aa-field textarea{min-height:78px;resize:vertical}.aa-actions{display:flex;flex-wrap:wrap;gap:9px;margin-top:12px}.aa-btn{border:1px solid #3a4165;background:#17192f;color:#e4e6f2;border-radius:9px;padding:9px 12px;font:11px inherit;cursor:pointer}.aa-btn.primary{background:linear-gradient(90deg,#1196fc,#5d0dfa);border:0;color:#fff}.aa-btn.danger{border-color:rgba(255,157,157,.35);background:rgba(255,157,157,.08);color:#ffb2b2}.aa-btn.warn{border-color:rgba(246,196,69,.36);background:rgba(246,196,69,.08);color:#f6d36a}.aa-btn.success{border-color:rgba(143,227,176,.32);background:rgba(143,227,176,.07);color:#9ceabb}.aa-btn:disabled{opacity:.45;cursor:not-allowed}.aa-note{font-size:10px;color:#8f95ad;margin-top:9px}.aa-states{display:grid;grid-template-columns:1fr 1fr;gap:8px;margin-bottom:13px}.aa-state{display:flex;align-items:center;gap:8px;padding:10px 12px;border:1px solid #282c47;border-radius:10px;background:#101225;font-size:11px}.aa-dot{width:8px;height:8px;border-radius:999px;background:#3F7C5B}.aa-dot.suspended{background:#B95C66}.aa-dot.takedown{background:#9A6D10}.aa-ok{font-size:11px;color:#3F7C5B;margin-top:10px}.aa-err{font-size:11px;color:#B95C66;margin-top:10px}@media(max-width:720px){.aa-grid,.aa-states{grid-template-columns:1fr}}`}</style>
