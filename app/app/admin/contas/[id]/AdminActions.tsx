...
export default function AdminActions({ userId, temStripe, suspenso = false }: { userId: string; temStripe: boolean; suspenso?: boolean }) {
  ...
  async function executar(acao: "plano_manual" | "sincronizar_stripe" | "suspender" | "reativar") {
    setMensagem(""); setErro("");
  ...
  return <section className="aa-card">
    ...
    <div className="aa-state"><span className={`aa-dot${suspenso?" suspended":""}`}/><strong>{suspenso ? "Conta suspensa administrativamente" : "Conta com acesso administrativo normal"}</strong></div>
    <div className="aa-grid">
      ...
      <div className="aa-field"><label>Motivo da ação</label><textarea value={motivo} onChange={e=>setMotivo(e.target.value)} placeholder="Ex.: revisão de abuso, correção operacional, acesso interno para suporte..."/></div>
    </div>
    ...
    <div className="aa-note">Suspender não cancela a assinatura Stripe, não apaga arquivos e não remove links públicos. Bloqueia o painel e novas operações autenticadas até reativação explícita.</div>
