...
export default function AdminContaPage() {
  ...
  `}</style><div className="wrap">
    ...
    {conta && <>
      {conta.suspensao?.ativa && <section className="susp"><strong>Conta suspensa administrativamente</strong><p>{conta.suspensao.motivo || "Sem motivo informado."} · desde {data(conta.suspensao.suspenso_em)}</p></section>}
      ...
      <AdminActions userId={conta.id} temStripe={Boolean(conta.assinatura?.provedor === "stripe" && conta.assinatura?.provedor_assinatura_id)} suspenso={Boolean(conta.suspensao?.ativa)} />
