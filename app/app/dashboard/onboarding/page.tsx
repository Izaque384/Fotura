...
export default function OnboardingPage() {
  ...
  return <main className="onb mf-shift">
    <MenuFotografo/>
    <style>{`
      .onb{min-height:100vh;box-sizing:border-box;background:linear-gradient(180deg,#F0EDF7,#ECE8F4);color:#21253A;padding:48px 5vw 80px}.onb-wrap{max-width:1040px;margin:0 auto}.ey{font-size:11px;letter-spacing:2px;text-transform:uppercase;color:#6f76a0}.onb h1{font-size:32px;margin:8px 0 8px;letter-spacing:-.8px}.sub{font-size:13px;color:#7a7f9a;line-height:1.65;max-width:680px}.progress{margin:28px 0 22px;padding:18px 20px;border:1px solid #DCD6EE;border-radius:14px;background:linear-gradient(180deg,#FAF8FD,#F3EFF9)}.prog-top{display:flex;justify-content:space-between;gap:16px;font-size:12px;color:#596079;margin-bottom:10px}.track{height:8px;border-radius:999px;background:#F0EDF7;overflow:hidden}.fill{height:100%;background:linear-gradient(90deg,#1196fc,#5d0dfa);transition:width .25s ease}.steps{display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:14px}.step{position:relative;padding:22px;border:1px solid #DCD6EE;border-radius:16px;background:linear-gradient(180deg,#FAF8FD,#F3EFF9);min-height:250px;display:flex;flex-direction:column}.num{width:30px;height:30px;border-radius:10px;display:grid;place-items:center;background:#E9E4F3;color:#8ea7ff;font-weight:800;font-size:12px;margin-bottom:18px}.step.done .num{background:rgba(143,227,176,.1);color:#3F7C5B}.step h2{font-size:17px;margin:0 0 8px}.step p{font-size:11px;line-height:1.6;color:#777e98;margin:0}.state{margin-top:13px;font-size:10px;font-weight:700;color:#8ea7ff}.step.done .state{color:#3F7C5B}.cta{margin-top:auto;padding-top:20px}.btn{display:inline-flex;justify-content:center;align-items:center;width:100%;box-sizing:border-box;border:1px solid #323650;border-radius:10px;background:#EEEAF8;color:#e3e6f3;padding:11px 13px;font:650 12px inherit;cursor:pointer}.btn.primary{border:0;background:linear-gradient(90deg,#1196fc,#5d0dfa);color:white}.done-note{margin-top:20px;padding:14px 16px;border-radius:12px;border:1px solid rgba(143,227,176,.18);background:rgba(143,227,176,.05);color:#a7d9ba;font-size:12px}.err{margin-top:20px;color:#B95C66;font-size:12px}@media(max-width:900px){.steps{grid-template-columns:1fr}.step{min-height:0}}@media(max-width:640px){.onb{padding:88px 18px 60px}.onb h1{font-size:27px}}
    `}</style>
    <div className="onb-wrap">
      ...
      <section className="steps">
        <article className={`step ${planoOk ? "done" : ""}`}><div className="num">{planoOk ? "✓" : "1"}</div><h2>Seu plano inicial</h2><p>Toda conta começa no Grátis. Quando precisar, faça upgrade apenas pelo armazenamento e pelos recursos avançados de apresentação.</p><div className="state">{planoOk ? `Plano ${estado?.planoNome} ativo` : "Ativando seu plano grátis"}</div><div className="cta"><button className={`btn ${!planoOk ? "primary" : ""}`} onClick={()=>router.push("/dashboard/assinatura")}>{planoOk ? "Ver planos" : "Ver planos"}</button></div></article>
