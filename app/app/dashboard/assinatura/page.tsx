...
export default function AssinaturaPage() {
  ...
  function escolher(plano: PlanoCodigo) {
    setAviso(`O plano ${PLANOS_FOTURA[plano].nome} já está preparado. O checkout será conectado ao gateway na próxima etapa.`);
  ...
  return <main className="bill-page mf-shift">
    ...
    <div className="bill-wrap">
      ...
      {carregando ? <div className="loading">Carregando assinatura…</div> : <>
        ...
        <div className="plans-head"><div><div className="bill-ey">Planos comerciais</div><h2>Escolha o nível certo para sua operação</h2></div><p>Valores mensais iniciais em reais.</p></div>
        <section className="plans">
          {comerciais.map((codigo) => { const p=PLANOS_FOTURA[codigo]; const ehAtual=atual===codigo; return <article key={codigo} className={`card plan${codigo==="profissional"?" featured":""}`}>
            ...
            <button disabled={ehAtual} className={`plan-btn${codigo==="profissional"?" primary":""}`} onClick={()=>escolher(codigo)}>{ehAtual?"Plano atual":"Escolher plano"}</button>
          </article>; })}
        </section>
        <p className="fine">Os preços e a estrutura já estão centralizados no catálogo de billing. A contratação só será efetivada quando o checkout e o webhook do gateway estiverem conectados.</p>
      </>}
    </div>
  </main>;
}
