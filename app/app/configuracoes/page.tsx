...
export default function ConfiguracoesPage() {
  ...
  useEffect(() => {
    ...
    void (async () => {
      ...
      if(session?.access_token){try{const r=await fetch("/api/google/integration",{headers:{Authorization:`Bearer ${session.access_token}`},cache:"no-store"});if(r.ok){const g=await r.json() as {conectado?:boolean;email?:string|null};if(ativo){setGoogleConectado(Boolean(g.conectado));setGoogleEmail(g.email??null)}}}catch{}}
  ...
  return <main className="cfg mf-shift"><MenuFotografo/><style>{`
    ...
    <div className="cfg-body"><div className="ey">CONFIGURAÇÕES</div><h1 className="h1">Conta e preferências</h1><p className="sub">Segurança, notificações, integrações e informações operacionais da sua conta.</p>
      <div className="grid">
        ...
        <section className="card"><div className="card-head"><div><h2 className="title">Google Contacts</h2><p className="desc">Use fotos dos seus Contatos Google nos cards de clientes quando o e-mail corresponder.</p></div><div className="icon" aria-hidden="true">G</div></div><div className="row"><div><div className="label">Conta Google</div><div className="value">{googleConectado?(googleEmail?`Conectada como ${googleEmail}`:"Conectada"):"Não conectada"}</div></div><button className={"btn "+(googleConectado?"off":"primary")} disabled={alterandoGoogle} onClick={()=>void (googleConectado?desconectarGoogle():conectarGoogle())}>{alterandoGoogle?"Processando…":googleConectado?"Desconectar":"Conectar Google"}</button></div></section>
