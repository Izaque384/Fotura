...
export async function GET(req:NextRequest){const a=await user(req);if(!a)return NextResponse.json({error:"Não autorizado."},{status:401});return NextResponse.json(await statusGoogle(a.user.id));}
