...
export async function GET(req: NextRequest) {
  ...
  const { data: g } = await auth.supabase
    .from("galerias")
  ...
  const { data: selecao } = await auth.supabase
    .from("selecoes")
  ...
  catch { return NextResponse.json({ error: "Não foi possível carregar a entrega final." }, { status: 500 }); }
...
export async function POST(req: NextRequest) {
  ...
  if (!uuidValido(galeria) || !body.acao) return NextResponse.json({ error: "Dados inválidos." }, { status: 400 });
  ...
  if (body.acao === "iniciar") {
    ...
    if (error) return NextResponse.json({ error: "Não foi possível iniciar a preparação da entrega." }, { status: 500 });
    return NextResponse.json({ ok: true, etapa: "preparando_entrega" });
  }

  let finais: string[];
  try { finais = await listarFinais(auth.supabase, auth.user.id, galeria); }
  catch { return NextResponse.json({ error: "Não foi possível validar as fotos finais." }, { status: 500 }); }
  ...
  if (error) return NextResponse.json({ error: "Não foi possível publicar a entrega." }, { status: 500 });
