...
export async function GET(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    const [perfilRes, assinaturaRes, suspensaoRes, takedownRes, encerramentoRes, galeriasRes, clientesRes, auditoriaRes, usoPlano] = await Promise.all([
      ...
      supabase.from("admin_encerramentos").select("status,motivo,solicitado_por,solicitado_em,elegivel_em,cancelado_por,cancelado_em,confirmado_por,confirmado_em,executado_por,executado_em,atualizado_em").eq("user_id", id).maybeSingle(),
