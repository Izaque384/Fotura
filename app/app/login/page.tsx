...
export default function LoginPage() {
  ...
  return (
    <main style={{ minHeight:"100vh",display:"flex",alignItems:"center",justifyContent:"center",background:"linear-gradient(180deg,#edf2f8 0%,#e7edf5 100%)",fontFamily:"sans-serif",padding:24 }}>
      <section aria-labelledby="login-title" style={{ background:"#f0f4f9",borderRadius:16,padding:"clamp(24px, 7vw, 40px)",width:"100%",maxWidth:400,border:"1px solid #d4dde9",boxSizing:"border-box" }}>
        <div aria-hidden="true" style={{ fontSize:28,fontWeight:700,letterSpacing:4,color:"#20253b",textAlign:"center",marginBottom:8 }}>FOTURA</div>
        <h1 id="login-title" style={{ fontSize:14,fontWeight:400,color:"#707990",textAlign:"center",margin:"0 0 32px" }}>{modo === "login" ? "Entre na sua conta" : "Crie sua conta"}</h1>

        <form onSubmit={(event) => { event.preventDefault(); void handleSubmit(); }} noValidate>
          <label htmlFor="login-email" style={{ fontSize:13,color:"#5f687e",display:"block",marginBottom:6 }}>E-mail</label>
          <input id="login-email" name="email" type="email" autoComplete="email" inputMode="email" value={email} onChange={(e)=>setEmail(e.target.value)} placeholder="seu@email.com" aria-invalid={mensagemErro && !email.trim() ? true : undefined} style={{ width:"100%",padding:"12px 14px",fontSize:14,border:"1.5px solid #d0dae7",borderRadius:10,background:"#f5f7fb",color:"#f0f0f5",outline:"none",marginBottom:20,boxSizing:"border-box" }} />

          <label htmlFor="login-senha" style={{ fontSize:13,color:"#5f687e",display:"block",marginBottom:6 }}>Senha</label>
          <input id="login-senha" name="senha" type="password" autoComplete={modo === "login" ? "current-password" : "new-password"} value={senha} onChange={(e)=>setSenha(e.target.value)} placeholder="••••••••" maxLength={128} aria-describedby={modo === "cadastro" ? "senha-ajuda" : undefined} aria-invalid={mensagemErro && !senha ? true : undefined} style={{ width:"100%",padding:"12px 14px",fontSize:14,border:"1.5px solid #d0dae7",borderRadius:10,background:"#f5f7fb",color:"#f0f0f5",outline:"none",marginBottom:12,boxSizing:"border-box" }} />
          {modo === "cadastro" && <p id="senha-ajuda" style={{fontSize:11,color:"#8a93a7",margin:"0 0 18px"}}>Mínimo de 6 caracteres.</p>}
          ...
          {modo === "cadastro" && (
            <label style={{display:"flex",alignItems:"flex-start",gap:10,marginBottom:24,cursor:"pointer"}}>
              ...
              <span style={{fontSize:12,color:"#5f687e",lineHeight:1.6}}>Li e aceito os <Link href="/termos" target="_blank" rel="noopener noreferrer" style={{color:"#4a6cf7",textDecoration:"underline"}}>Termos de Uso</Link> e a <Link href="/privacidade" target="_blank" rel="noopener noreferrer" style={{color:"#4a6cf7",textDecoration:"underline"}}>Política de Privacidade</Link>.</span>
            </label>
          )}
